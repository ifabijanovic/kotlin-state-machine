...
fun <Element> Observable<Element>.asDriver(onErrorJustReturn: Element): Driver<Element> {
  return SharedSequence(this.onErrorReturn { onErrorJustReturn }.observeOn(DriverTraits.scheduler),
                        DriverTraits.Companion)
}

fun <Element> Observable<Element>.asDriver(onErrorDriveWith: (Throwable) -> Driver<Element>): Driver<Element> {
  return SharedSequence(this.onErrorResumeNext { error: Throwable -> onErrorDriveWith(error).source }.observeOn(
      DriverTraits.scheduler), DriverTraits.Companion)
}

fun <Element> Observable<Element>.asDriverIgnoreError(): Driver<Element> {
  return SharedSequence(this.onErrorResumeNext { error: Throwable -> Observable.empty() }.observeOn(
      DriverTraits.scheduler), DriverTraits.Companion)
}
